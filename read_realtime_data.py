...
def main():
    ...
    try:
        realtime_data = client.read_holding_registers(slave=1, address=address, count=count)
    ...
    # Gebruik de registers direct
    registers = realtime_data.registers

    data = {
            "mpvmode": registers[0],
            "mpvstatus": DEVICE_STATUSSES.get(registers[0], "Unknown"),
            "pv1volt": round(registers[6] * 0.1, 1),
            "pv1curr": round(registers[7] * 0.01, 2),
            "pv1power": round(registers[8] * 1, 0),
